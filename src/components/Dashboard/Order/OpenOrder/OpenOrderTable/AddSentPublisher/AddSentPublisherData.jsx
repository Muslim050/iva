...
function AddSentPublisherData ({listsentPublisher, expandedRows, onceOrder, totalOnlineView}) {
        ...
        totalOnlineView += item?.online_views,
          ...
          <React.Fragment key={i}>
            {currentOrder === item ? (
              ...
              <tr>
                <th className={style.table__tr_th}>{item.publisher?.name}</th>
                <th className={style.table__tr_th}>{item.channel?.name}</th>
                {/*<th className={style.table__tr_th}>*/}
                {/*  <Link to={item.promo_file}>Видео</Link>*/}
                {/*</td>*/}
                <th
                  className={style.table__tr_th}>{(item.format === 'preroll' && 'Pre-roll') || ('mixroll' && 'Mix-roll')}</th>
                <th className={style.table__tr_th}>
                  ...
                  <div style={{display: "flex", gap: "5px",}}>
                    ...
                    {openPopoverIndex === i && (
                      <div style={{
                        ...
                        <PopoverEditView item={item} expandedRows={expandedRows}
                ...
                <th className={style.table__tr_th}>{item.age_range}</th>
                <th className={style.table__tr_th}>{item.content_language}</th>
                <th className={style.table__tr_th}>
                  <Link to={item.notes_url} style={{fontWeight: "600", color: "blue"}}>LINK</Link>
                </th>
                <th className={style.table__tr_th}>{item.country}</th>
